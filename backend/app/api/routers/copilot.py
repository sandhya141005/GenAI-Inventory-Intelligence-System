...
def chat(payload: ChatRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> CopilotResponse:
    ...
    conversation_service.add_message(conversation.id, "assistant", result["response"], {"intent": result["intent"]})
    db.commit()
    return CopilotResponse(conversation_id=conversation.id, **result)


@router.get("/morning-brief", response_model=CopilotResponse)
def morning_brief(current_user: User = Depends(get_current_user)) -> CopilotResponse:
...
def weekly_report(current_user: User = Depends(get_current_user)) -> CopilotResponse:
    return CopilotResponse(
        **run_copilot_workflow(
...
def recommendations(payload: RecommendationRequest, current_user: User = Depends(get_current_user)) -> CopilotResponse:
    prompt = (
    ...
    return CopilotResponse(
        **run_copilot_workflow(user_input=prompt, requested_intent="recommendations", user_id=current_user.id)
    )


@router.post("/nl-query", response_model=CopilotResponse)
def nl_query(payload: NLQueryRequest, current_user: User = Depends(get_current_user)) -> CopilotResponse:
    return CopilotResponse(
        **run_copilot_workflow(
