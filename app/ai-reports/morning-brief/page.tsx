...
export default function MorningBriefPage() {
  ...
  useEffect(() => {
    async function loadReport() {
      try {
        const data = await fetchAI<CopilotResponse>("/api/copilot/morning-brief");
        setReport(data);
      } catch (err) {
  ...
  function handleDownloadPDF() {
    ...
    doc.text("Morning Brief", margin, y);
    y += 20;
    ...
    doc.save(`morning-brief-${dateStamp}.pdf`);
  }
    ...
    <div className="max-w-4xl mx-auto px-6 py-8 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold text-ink">Morning Brief</h1>
            <p className="text-sm text-ink-muted">AI-generated inventory operations summary</p>
          </div>
        </div>
